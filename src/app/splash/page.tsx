...
function generateSplashPath(
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    const outerRadiusAdjustment =
      centerOffsetX * Math.cos(outerAngle) +
      centerOffsetY * Math.sin(outerAngle);
    const innerRadiusAdjustment =
      centerOffsetX * Math.cos(innerAngle) +
      centerOffsetY * Math.sin(innerAngle);
  ...
  if (useStraightLines) {
    ...
    if (outerCornerRadius === 0) {
      // Original behavior
      let d = `M ${points[0].outer.x.toFixed(2)} ${points[0].outer.y.toFixed(
        2
      )}`;
      for (let i = 0; i < numPoints; i++) {
        const next = (i + 1) % numPoints;
        d += ` Q ${points[i].inner.x.toFixed(2)} ${points[i].inner.y.toFixed(
          2
        )} ${points[next].outer.x.toFixed(2)} ${points[next].outer.y.toFixed(
          2
        )}`;
      }
      d += " Z";
      ...
      for (let i = 0; i < numPoints; i++) {
        ...
        d += ` Q ${curr.inner.x.toFixed(2)} ${curr.inner.y.toFixed(
          2
        )} ${nextPt.outer.x.toFixed(2)} ${nextPt.outer.y.toFixed(2)}`;
      }
      d += " Z";
      return d;
    }
...
function buildRoundedPath(
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (curr.radius > 0) {
      ...
      d += ` A ${curr.radius.toFixed(2)} ${curr.radius.toFixed(
        2
      )} 0 0 1 ${curr.end.x.toFixed(2)} ${curr.end.y.toFixed(2)}`;
    }
...
export default function SplashPage() {
  ...
  const updateConfig = useCallback(
    (key: keyof SplashConfig, value: number | string | boolean) => {
  ...
  return (
    <div className="h-dvh flex flex-col bg-background overflow-hidden">
      ...
      <div className="h-1/3 flex items-center justify-center bg-muted/20 border-b">
        ...
        >
          <path key={pathKey} d={path} fill={config.color} />
        </svg>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6">
        <div className="max-w-md mx-auto space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <ToggleGroup
                  ...
                  onValueChange={(value) => {
                    if (value)
                      updateConfig("useStraightLines", value === "straight");
                  }}
          ...
          <div className="pt-2 border-t">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                value={config.seed}
                onChange={(e) =>
                  updateConfig("seed", parseInt(e.target.value) || 0)
                }
          ...
          <div className="bg-muted/50 rounded-xl p-4 text-sm">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Higher ratio = sharper points. Try 2:1 to 3:1 for classic splash
              look.
            </p>
          </div>
        </div>
